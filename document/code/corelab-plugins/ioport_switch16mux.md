...
which displays a matrix of 16 push switches. The column to be read can be
selected directly by an active bit (up to 8 lines) or by specifying the line BCD
(up to 16 lines). The active bit can be high or low). The output returns the
switches status.
