...
`TStandardMemory` is the standard memory plugin of the CoreLAB framework. It is
derived from `TMemory` and provides a memory component configured for up to 16
MB of RAM or ROM operation.

### Protected fields

`TStandardMemory` does not declare additional protected fields. It uses the
inherited fields of `TMemory`.
