...
`ISvcAPI` is the CoreLAB service API interface. It groups common service
operations used by the supervisor for CPU, I/O-port and memory components.

The source distinguishes operations implemented by all used classes from
operations intended specifically for `TMemory` or `TGIOPort`.
...
The source comment identifies `ISvcAPI` as the service path from `TSupervisor`
to `TCPU`, `TIOPort` and `TMemory`.

`TBus` implements `ISvcAPI` and therefore exposes the service operations at the
 bus level as well.
