...
`TIOPort` is an abstract base class for I/O-port device implementations. It
defines common address-range, data-line decoding, interrupt, state-persistence,
and module-identification properties. Concrete devices must implement reset,
port read, and port write operations.
