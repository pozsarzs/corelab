...
`ISysBus` is the CoreLAB system-bus interface. It defines the memory and I/O operations used by a CPU to communicate with memory and I/O-port devices through the system bus.

The source identifies the interface as the CPU-to-device path from `TCPU` to `TIOPort` and `TMemory`.
...
The source comment identifies `ISysBus` as the communication path from `TCPU` to `TIOPort` and `TMemory`.

`TBus` implements `ISysBus` and therefore provides the bus-level implementation of these operations.
