...
`TLED` is an abstract base class for graphical LED implementations. It stores the LED state, color set, background color, and an internal bitmap buffer. Derived classes implement drawing the LED into the buffer and rendering it onto a target canvas.
...
`SetColor`, `SetBGColor`, and `SetOn` immediately call `DrawToBuffer` with the current state and updated value.

`Reset` sets `IsOn` to `false` and redraws.

The constructor creates the bitmap buffer but does not explicitly initialize the other protected fields.
