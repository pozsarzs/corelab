...
`TCommandEngine` is the command execution engine. It tokenizes command lines,
resolves command names through a command registry, creates command objects,
executes them in the current running mode, executes associated actions and
stores the resulting exit code.
