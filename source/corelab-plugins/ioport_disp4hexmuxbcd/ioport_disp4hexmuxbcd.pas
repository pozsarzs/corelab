...
// CREATE TDISP4HEXMUXBCD INSTANCE
constructor TDisp4HexMUXBCD.Create;
var
  b: Byte;
begin
  inherited Create;
  FModname := '4-digit hexadecimal multiplexed display with BCD input';
...
// DESTROY TDISP4HEXMUXBCD INSTANCE
...
// READ VIRTUAL PORT
function TDisp4HexMUXBCD.ReadPort(APort: Word): Byte;
begin
  if FEnabled then Result := 0 else Result := $FF;
end;

// WRITE VIRTUAL PORT
procedure TDisp4HexMUXBCD.WritePort(APort: Word; AValue: Byte);
begin
  if FEnabled then 
    case APort of
      0: begin
...
// LOAD SAVED STATE
function TDisp4HexMUXBCD.LoadState(AStream: TStream): Boolean;
var
  b: Byte;
begin
...
// SAVE ACTUAL STATE
function TDisp4HexMUXBCD.SaveState(AStream: TStream): Boolean;
var
  b: Byte;
begin
...
// CREATE PANEL
procedure TDisp4HexMUXBCD.CreatePanel;
begin
...
begin
  if Assigned(APort) and (APort is TDisp4HexMUXBCD)
    then TDisp4HexMUXBCD(APort).CreatePanel;
end;
