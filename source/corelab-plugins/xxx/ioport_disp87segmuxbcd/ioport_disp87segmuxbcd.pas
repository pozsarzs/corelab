...
library ioport_disp87segmuxbcd;
{$mode objfpc}{$H+}
uses
...
type
  ...
  TDisp87segMuxBCD = class(TIOPort)
  protected
...
procedure TDisp87segMuxBCD.PaintBoxPaint(Sender: TObject);
var
  b: byte;
begin
...
constructor TDisp87segMuxBCD.Create;
var
  b: byte;
  s: string;
begin
  inherited Create;
  s := '8 pcs. 7 segments display with BCD input';
  FModname := PChar(s);
...
destructor TDisp87segMuxBCD.Destroy;
var
  b: byte;
begin
...
// Read virtual port
function TDisp87segMuxBCD.ReadPort(Port: byte): byte;
begin
  Result := 0;
end;

// Write virtual port
procedure TDisp87segMuxBCD.WritePort(Port: byte; Value: byte);
begin
  case Port of
...
// Reset virtual port
procedure TDisp87segMuxBCD.Reset;
var
  b: byte;
begin
...
begin
  Result := TDisp87segMuxBCD.Create;
end;
...
begin
  ...
  PaintBox.OnPaint := @TDisp87segMuxBCD(Port).PaintBoxPaint;
