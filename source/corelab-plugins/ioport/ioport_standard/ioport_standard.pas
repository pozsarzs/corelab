...
type
  ...
  var
    CurrentPort: TStdPort = nil;    
    EditRx: TEdit = nil;
    EditTx: TEdit = nil;
    PanelForm: TForm = nil;
  
// Create TIOPort instance
constructor TStdPort.Create;
begin
  inherited Create;
...
destructor TStdPort.Destroy;
begin
  inherited Destroy;
end;

// Read virtual port
function TStdPort.ReadPort(Port: byte): byte;
var
  Value: integer;
begin
  Result := 0;
...
// Write virtual port
procedure TStdPort.WritePort(Port: byte; Value: byte);
begin
  if Assigned(EditTx) then EditTx.Text := IntToHex(Value, 2);
end;

// Reset virtual port
procedure TStdPort.Reset;
begin
