...
// READ VIRTUAL PORT
function TDisp1HexBCD.ReadPort(APort: Word): Byte;
begin
  if FEnabled and (APort = 0) then Result := 0 else Result := $FF;
end;

// WRITE VIRTUAL PORT
procedure TDisp1HexBCD.WritePort(APort: Word; AValue: Byte);
begin
...
// LOAD SAVED STATE
function TDisp1HexBCD.LoadState(AStream: TStream): Boolean;
begin
...
// SAVE ACTUAL STATE
function TDisp1HexBCD.SaveState(AStream: TStream): Boolean;
begin
...
begin
  if Assigned(APort) and (APort is TDisp1HexBCD)
    then TDisp1HexBCD(APort).CreatePanel;
end;
