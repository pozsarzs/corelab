...
procedure TLEDRed512MUX.PaintBoxPaint(Sender: TObject);
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// CREATE TLEDRED512MUX INSTANCE
constructor TLEDRed512MUX.Create;
var
  x, y: Byte;
begin
  inherited Create;
...
// DESTROY TLEDRED512MUX INSTANCE
destructor TLEDRed512MUX.Destroy;
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// READ VIRTUAL PORT
function TLEDRed512MUX.ReadPort(APort: Word): Byte;
begin
  if FEnabled then Result := 0 else Result := $FF;
end;

// WRITE VIRTUAL PORT
procedure TLEDRed512MUX.WritePort(APort: Word; AValue: Byte);
var
  y:     Byte;
  Value: Byte;
begin
...
// LOAD SAVED STATE
function TLEDRed512MUX.LoadState(AStream: TStream): Boolean;
var
  x: Byte;
begin
...
// SAVE ACTUAL STATE
function TLEDRed512MUX.SaveState(AStream: TStream): Boolean;
var
  x: Byte;
begin
...
// CREATE PANEL
procedure TLEDRed512MUX.CreatePanel;
begin
...
begin
  if Assigned(APort) and (APort is TLEDRed512MUX)
    then TLEDRed512MUX(APort).CreatePanel;
end;
