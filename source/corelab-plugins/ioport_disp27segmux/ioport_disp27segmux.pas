...
type
  // 7 segment display output implementation
  TDisp27segMux = class(TIOPort)
  protected
...
begin
  inherited Create;
  s := '2 pcs. 7 segments multiplexed display';
  FModname := PChar(s);
...
// Read virtual port
function TDisp27segMux.ReadPort(Port: byte): byte;
begin
  Result := 0;
end;

// Write virtual port
procedure TDisp27segMux.WritePort(Port: byte; Value: byte);
begin
  case Port of
...
begin
  ...
  PaintBox.OnPaint := @TDisp27segMux(Port).PaintBoxPaint;
