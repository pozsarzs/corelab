...
type
  { TLEDYellow64MUX }
  TLEDYellow64MUX = class(TGIOPort)
  private
...
procedure TLEDYellow64MUX.PaintBoxPaint(Sender: TObject);
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// CREATE TLEDYELLOW64MUX INSTANCE
constructor TLEDYellow64MUX.Create;
var
  x, y: Byte;
begin
  inherited Create;
...
// DESTROY TLEDYELLOW64MUX INSTANCE
destructor TLEDYellow64MUX.Destroy;
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// RESET VIRTUAL PORT
procedure TLEDYellow64MUX.Reset;
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// READ VIRTUAL PORT
function TLEDYellow64MUX.ReadPort(APort: Word): Byte;
begin
  if FEnabled then Result := 0 else Result := $FF;
end;

// WRITE VIRTUAL PORT
procedure TLEDYellow64MUX.WritePort(APort: Word; AValue: Byte);
var
  y:     Byte;
  Value: Byte;
begin
...
// LOAD SAVED STATE
function TLEDYellow64MUX.LoadState(AStream: TStream): Boolean;
var
  x: Byte;
begin
...
// SAVE ACTUAL STATE
function TLEDYellow64MUX.SaveState(AStream: TStream): Boolean;
var
  x: Byte;
begin
...
// CREATE PANEL
procedure TLEDYellow64MUX.CreatePanel;
begin
...
begin
  if Assigned(APort) and (APort is TLEDYellow64MUX)
    then TLEDYellow64MUX(APort).CreatePanel;
end;
