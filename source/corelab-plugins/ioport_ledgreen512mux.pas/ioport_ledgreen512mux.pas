...
type
  ...
  TLEDGreen512MUX = class(TGIOPort)
  private
...
procedure TLEDGreen512MUX.PaintBoxPaint(Sender: TObject);
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// CREATE TLEDGREEN512MUX INSTANCE
constructor TLEDGreen512MUX.Create;
var
  x, y: Byte;
begin
  inherited Create;
  ...
  begin
    ...
    with FLED[x, y] do
    begin
      Color := RETRO_COLORS[clRetroGreen];
...
// DESTROY TLEDGREEN512MUX INSTANCE
destructor TLEDGreen512MUX.Destroy;
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// RESET VIRTUAL PORT
procedure TLEDGreen512MUX.Reset;
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// READ VIRTUAL PORT
function TLEDGreen512MUX.ReadPort(APort: Word): Byte;
begin
  if FEnabled then Result := 0 else Result := $FF;
end;

// WRITE VIRTUAL PORT
procedure TLEDGreen512MUX.WritePort(APort: Word; AValue: Byte);
var
  y:     Byte;
  Value: Byte;
begin
...
// LOAD SAVED STATE
function TLEDGreen512MUX.LoadState(AStream: TStream): Boolean;
var
  x: Byte;
begin
...
// SAVE ACTUAL STATE
function TLEDGreen512MUX.SaveState(AStream: TStream): Boolean;
var
  x: Byte;
begin
...
// CREATE PANEL
procedure TLEDGreen512MUX.CreatePanel;
begin
...
begin
  if Assigned(APort) and (APort is TLEDGreen512MUX)
    then TLEDGreen512MUX(APort).CreatePanel;
end;
