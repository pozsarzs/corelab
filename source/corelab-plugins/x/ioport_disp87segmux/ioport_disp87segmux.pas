...
// READ VIRTUAL PORT
function TDisp87segMUX.ReadPort(Port: byte): byte;
begin
...
// WRITE VIRTUAL PORT
procedure TDisp87segMUX.WritePort(Port: byte; Value: byte);
begin
  if FEnabled then 
  case Port of
...
begin
  if Assigned(Port) and (Port is TDisp87segMUX) then
    TDisp87segMUX(Port).CreatePanel;
end;
