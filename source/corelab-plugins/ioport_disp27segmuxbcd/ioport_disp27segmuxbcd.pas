...
library ioport_disp27segmuxbcd;
{$mode objfpc}{$H+}
uses
...
type
  ...
  TDisp27segMuxBCD = class(TIOPort)
  protected
...
procedure TDisp27segMuxBCD.PaintBoxPaint(Sender: TObject);
var
  b: byte;
begin
...
constructor TDisp27segMuxBCD.Create;
var
  b: byte;
  s: string;
begin
  inherited Create;
  s := '2 pcs. 7 segments display with BCD input';
  FModname := PChar(s);
...
destructor TDisp27segMuxBCD.Destroy;
var
  b: byte;
begin
...
// Read virtual port
function TDisp27segMuxBCD.ReadPort(Port: byte): byte;
begin
  Result := 0;
end;

// Write virtual port
procedure TDisp27segMuxBCD.WritePort(Port: byte; Value: byte);
begin
  case Port of
...
// Reset virtual port
procedure TDisp27segMuxBCD.Reset;
var
  b: byte;
begin
...
begin
  Result := TDisp27segMuxBCD.Create;
end;
...
begin
  ...
  PaintBox.OnPaint := @TDisp27segMuxBCD(Port).PaintBoxPaint;
