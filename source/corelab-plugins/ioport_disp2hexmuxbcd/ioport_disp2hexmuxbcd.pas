...
// READ VIRTUAL PORT
function TDisp2HexMUXBCD.ReadPort(APort: Word): Byte;
begin
  if FEnabled then Result := 0 else Result := $FF;
end;

// WRITE VIRTUAL PORT
procedure TDisp2HexMUXBCD.WritePort(APort: Word; AValue: Byte);
begin
  if FEnabled then 
    case APort of
      0: begin
...
// LOAD SAVED STATE
function TDisp2HexMUXBCD.LoadState(AStream: TStream): Boolean;
var
  b: Byte;
begin
...
// SAVE ACTUAL STATE
function TDisp2HexMUXBCD.SaveState(AStream: TStream): Boolean;
var
  b: Byte;
begin
...
begin
  if Assigned(APort) and (APort is TDisp2HexMUXBCD)
    then TDisp2HexMUXBCD(APort).CreatePanel;
end;
