...
procedure TLEDRed64MUX.PaintBoxPaint(Sender: TObject);
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// CREATE TLEDRED64MUX INSTANCE
constructor TLEDRed64MUX.Create;
var
  x, y: Byte;
begin
  inherited Create;
...
// DESTROY TLEDRED64MUX INSTANCE
destructor TLEDRed64MUX.Destroy;
var
  x, y: Byte;
begin
  for x := 0 to MAXX do
...
// READ VIRTUAL PORT
function TLEDRed64MUX.ReadPort(APort: Word): Byte;
begin
  if FEnabled then Result := 0 else Result := $FF;
end;

// WRITE VIRTUAL PORT
procedure TLEDRed64MUX.WritePort(APort: Word; AValue: Byte);
var
  y:     Byte;
  Value: Byte;
begin
...
// LOAD SAVED STATE
function TLEDRed64MUX.LoadState(AStream: TStream): Boolean;
var
  x: Byte;
begin
...
// SAVE ACTUAL STATE
function TLEDRed64MUX.SaveState(AStream: TStream): Boolean;
var
  x: Byte;
begin
...
// CREATE PANEL
procedure TLEDRed64MUX.CreatePanel;
begin
...
begin
  if Assigned(APort) and (APort is TLEDRed64MUX)
    then TLEDRed64MUX(APort).CreatePanel;
end;
