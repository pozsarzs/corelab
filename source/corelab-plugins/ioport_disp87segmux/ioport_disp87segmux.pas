...
procedure TDisp87segMUX.PaintBoxPaint(Sender: TObject);
var
  b: Byte;
begin
...
// CREATE TDISP87SEGMUX INSTANCE
constructor TDisp87segMUX.Create;
var
  b: Byte;
begin
  inherited Create;
  FModname := '8-digit 7 segments multiplexed display';
...
// DESTROY TDISP87SEGMUX INSTANCE
destructor TDisp87segMUX.Destroy;
var
  b: Byte;
begin
...
// READ VIRTUAL PORT
function TDisp87segMUX.ReadPort(APort: Word): Byte;
begin
  if FEnabled then Result := 0 else Result := $FF;
end;

// WRITE VIRTUAL PORT
procedure TDisp87segMUX.WritePort(APort: Word; AValue: Byte);
begin
  if FEnabled then 
  case APort of
    0: begin
...
// LOAD SAVED STATE
function TDisp87segMUX.LoadState(AStream: TStream): Boolean;
var
  b: Byte;
begin
...
// SAVE ACTUAL STATE
function TDisp87segMUX.SaveState(AStream: TStream): Boolean;
var
  b: Byte;
begin
...
// CREATE PANEL
procedure TDisp87segMUX.CreatePanel;
begin
...
begin
  if Assigned(APort) and (APort is TDisp87segMUX)
    then TDisp87segMUX(APort).CreatePanel;
end;
