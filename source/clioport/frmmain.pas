...
type
  ...
  private
    ...
    CreatePort:       TCreatePortFunc;                     // create plugin port
    DestroyPort:      TDestroyPortProc;                   // destroy plugin port
    CreatePanel:      TCreatePanelProc;                   // create plugin panel
    ShowPanel:        TShowPanelProc;                       // show plugin panel
    ...
    procedure InterruptHandler(Sender: TIOPort; Vector: Byte);
...
// INTERRUPT HANDLER
procedure TForm1.InterruptHandler(Sender: TIOPort; Vector: Byte);
begin
  StatusBar1.Panels.Items[2].Text := Format(MSG27, [IntToHex(Vector, 2)]);
  Timer1.Enabled := True;
end;
...
begin
  ...
  begin
    ...
    begin
      SetIntHandler(CurrentPort, nil, 0);
    ...
    // module
    Pointer(SetIntHandler) := GetProcedureAddress(LibHandle, 'ioport_setinthandler');
    Pointer(LoadState) := GetProcedureAddress(LibHandle, 'ioport_loadstate');
    Pointer(SaveState) := GetProcedureAddress(LibHandle, 'ioport_savestate');
    // load data
...
begin
  if LoadedPlugin.PHasPanel and Assigned(ShowPanel) then
  begin
    ShowPanel(CurrentPort);
  end;
end;
...
begin
  if LoadedPlugin.PHasPanel and Assigned(CreatePanel) then
  begin
    with Form3 do
    begin
...
begin
  ...
  begin
    ...
    try
      if not SaveState(CurrentPort, SaveStream) then ShowMessage(MSG01 + MSG20) else
        try
...
begin
  // panel
  if LoadedPlugin.PHasPanel and Assigned(FreePanel) then FreePanel(CurrentPort);
  // port
  if Assigned(CurrentPort) then
  begin
    SetIntHandler(CurrentPort, nil, 0);
